...
### C3 3D asset validation スモーク（5〜15分目安）

3D メッシュ / テクスチャ / shader / material 契約の最小診断を 1 コマンドで再実行する手順です。
...
確認ポイント:
- `python3 tools/validate_3d_assets.py` が `RESULT: PASS` を返すこと
- 不正 fixture `tools/tests/fixtures/invalid_missing_faces.obj.fixture` に対して `RESULT: FAIL` と `no drawable faces found` が出力されること
- `artifacts/c3_3d_asset_validation_latest.log` に最新の summary が残ること
