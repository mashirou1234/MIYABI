...
- 破損検知時の退避ファイルは `save_data.json.bak`, `save_data.json.bak.1`, `save_data.json.bak.2` ... の連番で増える（`logic/src/save.rs` の実装準拠）。
- 復旧判断に必要な履歴を確保するため、最新 3 世代（`.bak` / `.bak.1` / `.bak.2`）は常に保持する。
- 4 世代目以降（`.bak.3` 以降）は容量管理対象とし、障害調査が完了した時点で古い世代から削除してよい。
- 保存失敗やバージョン不一致のみで、破損が確認されないケースではバックアップ世代整理を行わない。
