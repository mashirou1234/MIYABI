...
### 3.3 再import時の `asset_id` 保持条件

再import（`ReloadTexture`）で `asset_id` を保持する条件は以下の 2 点を **同時に満たす** 場合に限定する。

1. `texture_handle_map` に対象 `path` が既に登録され、`texture_asset_id_map` から `asset_id` を逆引きできること
2. 実行コマンドが新規 import（`LoadTexture`）ではなく、既存ハンドル向けの `ReloadTexture` であること

この条件を満たす場合、`asset_id` は保持し、更新対象は GPU テクスチャ内容のみとする。

`asset_id` を再採番する（保持しない）ケース:
- 新規パスを初回 import した場合（`LoadTexture`）
- ファイルのリネーム/移動で `path` が変わり、既存 `path` と一致しない場合
- レジストリ不整合から復旧するために対象アセットを再登録した場合

運用上の判断基準:
- 「見た目は同じ画像でも `path` が同一でない限り別アセット」として扱う
- ID 継続が必要な差し替えは、ファイル名を変えずに上書きして `U` キー reimport を実行する
