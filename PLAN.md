...
-   [ ] **タスク2.1: 基本的なシェーダとマテリアルの導入**
    -   [x] Rust側でマテリアル情報を定義できるようにする（例: シェーダの種類、色、テクスチャIDなど）。
    -   [ ] C++側で、受け取ったマテリアル情報に基づいてシェーダを切り替え、描画できるようにする。
    -   [ ] 単純な単色シェーダと、テクスチャマッピング用のシェーダを実装する。

-   [ ] **タスク2.2: テクスチャの読み込みと管理**
    -   [ ] Rust側からテクスチャのロードを指示するコマンドを定義する。
    -   [ ] C++側で、画像ファイル（PNGなど）を読み込み、OpenGLのテクスチャオブジェクトとしてGPUにアップロードする機構を実装する。
    -   [ ] RustはテクスチャIDを通じて描画を指示できるようにする。

## フェーズ3: ゲームロジックとインタラクション

基本的な描画が可能になったところで、ゲームのロジックを実装します。

-   [ ] **タスク3.1: ECSの高度化とシーン構築API**
    -   [ ] `PLAN.md`にある宣言的シーンマクロのアイデアを具体化し、`world.spawn()`のようなAPIを整備する。
    -   [ ] 位置や速度といった基本的なコンポーネントを更新する簡単なシステム（System）をRustで記述する。
    -   [ ] フレームごとにRustの`world.update()`のような関数を呼び出し、C++へ描画データを渡すメインループを完成させる。
